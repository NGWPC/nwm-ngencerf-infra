...
Creates the Terraform state backend (S3 bucket + customer-managed KMS key) that every env's root module under `aws/envs/<env>/` uses to store its state. One bucket per AWS account, shared across all envs in that account; envs use different state keys (`personal-dev/terraform.tfstate`, `dev/terraform.tfstate`, `ea/terraform.tfstate`, etc.).

State **locking** uses S3's native lock-file mechanism (`use_lockfile = true` in the backend config). **DynamoDB is NOT used** — that pattern is deprecated as of Terraform 1.10. References:
...
For each env in this account. Current layout: the personal AWS account holds `personal-dev`; the NGWPC Test account holds `test/dev`, `test/dev2`, `test/perf`, `test/integration`; the NGWPC Optimization account holds `optimization/ea`, `optimization/uat`, `optimization/uat2`. NGWPC's two state buckets (`ngwpc-infra-test` for the Test account and `ngwpc-infra-oe` for the Optimization account — both match nomad-runner's `{account-short-name}` pattern) are pre-existing in the NGWPC infra org; how they were provisioned (LZA vs manual vs another repo) is unverified. The bootstrap module is only ever run in the personal account.
